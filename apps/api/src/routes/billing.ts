...
router.post("/invoices/:id/issue", requireRole("admin", "finance_officer"), (req, res, next) => {
  try {
...
router.post("/invoices/:id/payments", requireRole("admin", "finance_officer"), (req, res, next) => {
  try {
    const inv = db.prepare(`SELECT * FROM invoices WHERE id = ?`).get(req.params.id) as any;
    if (!inv) return res.status(404).json({ error: "Invoice not found" });
