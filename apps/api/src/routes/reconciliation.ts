...
router.post("/run", requireRole("admin", "airport_fuel_manager", "national_fuel_manager", "finance_officer"), (req, res, next) => {
  try {
...
router.post("/:id/approve", requireRole("admin", "airport_fuel_manager", "national_fuel_manager"), (req, res, next) => {
  try {
