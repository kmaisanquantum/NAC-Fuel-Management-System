...
for (const name of SUPPLIERS) {
  ...
  `).run(id, name, `contracts@${name.toLowerCase().replace(/[^a-z]/g, "")}.example`, `FMS-SUP-${1000 + supplierIds.length}`);
}
...
for (const [code] of AIRPORTS) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    `).run(id, airportIds[code], `BOW-${code}-${i + 1}`, `FMS-BOW-${1000 + rIdx}`, 40000, jetA1Id, 15000);
  }
}
...
console.log("Demo login: admin@dspng.tech / Admin@123!  (DEMO / NOT REAL NAC DATA)");
